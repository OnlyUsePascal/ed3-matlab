...
% Time parameters
dt = 0.1; % Time step (s)
t_total = 3; % Total time for simulation (s)
...
x_pos = 0; % Start on the circle
y_pos = 0; % Start at y = 0
phi = pi/2; % Orientation (rad)
...
% ========================= Video Writer Setup ===========================
video = VideoWriter('Circular_Trajectory_with_Aligned_Wheels.avi'); % Create video file
...
for t = 1:(t_steps + 5) % Additional 5 steps for 0.5-second delay
    ...
    if t <= 5
        ...
        x_vel = linear_velocity * cos(angular_velocity * time(t - 5)); % X velocity (forward motion)
        y_vel = linear_velocity * sin(angular_velocity * time(t - 5)); % Y velocity
        phi_vel = angular_velocity; % No rotation for the robot body
    end
...
xlabel('Time (s)');
ylabel('Velocity (m/s)');
title('Combined Wheel Velocities');
legend('show');
